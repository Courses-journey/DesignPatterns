...
import 'memento/care_taker.dart';
...
int main() {
  while (true) {
    ...
    while (true) {
      ...
      stdout.writeln("\t8. Save Memento");
      stdout.writeln("\t9. Restore Memento");
      ...
      if (itemId > 0 && itemId <= productList.length) {
        ...
        int mementoIndex = careTaker.addMemento(order.saveStateToMemento());
        printColor(
          "Memento saved at index #${mementoIndex}",
          color: Colors.yellow,
        );
        printOrder(order);
      } else if (itemId == 9) {
        printColor("Enter Memento Index: ", color: Colors.purple);
        int mementoIndex = int.parse(stdin.readLineSync() ?? "0");
        final OrderMemento orderMemento = careTaker.getMemento(
          mementoIndex,
        );
        order.restoreStateFromMemento(orderMemento);
        printOrder(order);
      }
